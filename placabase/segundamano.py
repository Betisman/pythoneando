...
pretexto = "/li?q="
posttexto = "&ca=28_s&c=3&x=2&w=2&z="
links = []

def urlizarTexto(texto):
	texto = texto.replace(' ', '+')
	return texto

def getHtmlBusqueda(texto):
	url = host + pretexto + urlizarTexto(texto) + posttexto
...
texto = raw_input('Texto a buscar: ')
html = getHtmlBusqueda(texto)
table = getTable(html)
mostrar(table)
